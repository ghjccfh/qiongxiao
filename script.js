...
if(confirm(`确定 ${targetValue}（允许±10%偏差）？`)){const result=await resetUnopenedBoxesByValue(targetValue);if(result.success){showMessage(result.message,'success')}else{showMessage(result.message,'error')}}
closeAdminPanel()})}
...
box.presetItemId=selectedItemId;await saveRoundToSupabase(gameData.globalRound);renderTreasureBoxesForAdmin();let message='';if(selectedItemId===0){message=`宝盒 #${box.id} `;if(oldItem){message+=`，物品 ${oldItem.name} 已放回物品池`}}else{const item=gameData.items.find(i=>i.id===selectedItemId);if(item){message=`宝盒 #${box.id} 已更换为 ${item.name} (${item.value}金币)`;if(oldItem){message+=`，原物品 ${oldItem.name} 已放回物品池`}}}
...
async function resetUnopenedBoxes(){try{showMessage('正在重置...','info');const{data:result,error}=await supabase.rpc('reset_unopened_boxes_by_admin');if(error){console.error('重置失败:',error);showMessage('失败重置：'+error.message,'error');return}
if(result&&!result.success){showMessage(result.message||'重置失败','error');return}
const{data:updatedRound,error:updateError}=await supabase.from('global_rounds').select('*').eq('id',gameData.globalRound.id).single();if(updateError){console.error('重新加载轮次数据失败:',updateError);showMessage('成功但更新显示失败','warning');return}重置
...
async function resetUnopenedBoxesByValue(targetValue){try{showMessage(`正在按价值 ${targetValue} 重置...`,'info');const{data,error}=await supabase.rpc('reset_unopened_boxes_by_value',{target_value:targetValue,tolerance_percent:10,enable_rollback:!0});if(error){throw error}
