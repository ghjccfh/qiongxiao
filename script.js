...
console.log('初始化游戏...');showMessage('正在加载游戏数据...','info');await loadGameData();function bindModeButtons(){const modeBtns=document.querySelectorAll('.mode-btn');modeBtns.forEach(btn=>{btn.addEventListener('click',()=>{const mode=parseInt(btn.dataset.mode);if(isNaN(mode))return;currentBoxMode=mode;modeBtns.forEach(b=>b.classList.remove('active'));btn.classList.add('active');const modeName=mode===1?'普通模式':mode===2?'高级模式':'豪华模式';showMessage(`已切换到 ${modeName}`,'info')})});const defaultBtn=document.querySelector('.mode-btn[data-mode="1"]');if(defaultBtn)defaultBtn.classList.add('active');}
