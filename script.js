let coinOperationsCurrentPage=1;let coinOperationsPageSize=20;let coinOperationsFilteredData=[];let isBoxOpening=!1;let isSavingItem=!1;let bgMusic=null;let isMusicPlaying=!1;let isResettingMode=!1;let isResettingInProgress=!1;let skipRealtimeRender=!1;let currentBoxMode=1;let chatMessages=[];let onlineUsers=[];let heartbeatInterval=null;let chatSubscription=null;let onlineUsersSubscription=null;let unreadChatCount=0;window._lastProcessedRoundId=0;window._isLoadingNewRound=!1;function updateChatUnreadBadge(){const badge=document.getElementById('chatUnreadBadge');if(badge){badge.textContent=unreadChatCount;badge.style.display=unreadChatCount>0?'inline-block':'none'}}
...
if(gameData.currentUser&&(gameData.currentUser.role==='admin'||gameData.currentUser.role==='subadmin')){populateCoinsUserSelect();updateAdminData()}}}else if(eventType==='DELETE'){if(oldData&&oldData.id){gameData.users=gameData.users.filter(u=>u.id!==oldData.id);if(gameData.currentUser&&gameData.currentUser.id===oldData.id){handleLogout()}}}}catch(err){console.error('handleUsersUpdate 出错:',err,{eventType,newData,oldData})}}
...
function showPage(page){if(page==='home'){const loginRequired=document.getElementById('loginRequired');const gameArea=document.getElementById('gameArea');if(!gameData.currentUser){if(loginRequired)loginRequired.style.display='block';if(gameArea)gameArea.style.display='none';const adminHomeStats=document.getElementById('adminHomeStats');if(adminHomeStats)adminHomeStats.style.display='none'}else{if(loginRequired)loginRequired.style.display='none';if(gameArea)gameArea.style.display='block'}}
const targetSection=document.getElementById(`${page}-section`);if(targetSection){targetSection.classList.add('active');updatePageVisibility(page);switch(page){case 'chat':if(gameData.currentUser){document.getElementById('chatContent').style.display='block';document.getElementById('chatLoginRequired').style.display='none';const messagesList=document.getElementById('chatMessagesList');if(messagesList){messagesList.innerHTML='<div style="text-align:center; padding:20px;"><i class="fas fa-spinner fa-spin"></i> 加载聊天记录...</div>'}
...
if(needRefresh){updateUserUI()}}).subscribe((status)=>{if(status==='SUBSCRIBED'){console.log('✅ 已订阅当前用户余额变化')}else if(status==='CHANNEL_ERROR'){console.warn('❌ 余额订阅通道错误，5秒后重试');setTimeout(()=>subscribeToOwnUserUpdates(),5000)}})}
...
allowedUsers.forEach(user=>{const option=document.createElement('option');option.value=user.id;const balance=user.balance??0;const giftBalance=user.gift_balance??0;option.textContent=`${user.username} (普通:${balance} 赠送:${giftBalance})`;coinsUserSelect.appendChild(option)})}
