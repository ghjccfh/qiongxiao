...
async function openBox(boxId){console.log('【调试】当前开盒模式:',currentBoxMode);const costMap={1:20000,2:100000,3:200000};const requiredCoins=costMap[currentBoxMode];const totalBalance=(gameData.currentUser.balance||0)+(gameData.currentUser.gift_balance||0);if(totalBalance<requiredCoins){showMessage(`金币不足，${currentBoxMode === 1 ? '普通' : currentBoxMode === 2 ? '高级' : '豪华'}模式需要 ${requiredCoins} 金币（普通余额: ${gameData.currentUser.balance}，赠送余额: ${gameData.currentUser.gift_balance}）`,'error');isBoxOpening=!1;enableAllBoxes();return}
