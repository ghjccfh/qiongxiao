...
console.log('初始化游戏...');showMessage('正在加载游戏数据...','info');await loadGameData();function bindModeButtons(){const modeBtns=document.querySelectorAll('.mode-btn');modeBtns.forEach(btn=>{btn.addEventListener('click',function(e){e.preventDefault();const mode=parseInt(this.dataset.mode);if(isNaN(mode))return;currentBoxMode=mode;modeBtns.forEach(b=>b.classList.remove('active'));this.classList.add('active');showMessage(`已切换到${mode === 1 ? '普通' : mode === 2 ? '高级' : '豪华'}模式`,'info')})});const defaultBtn=document.querySelector('.mode-btn[data-mode="1"]');if(defaultBtn)defaultBtn.classList.add('active');}
...
bindEvents();updateWarehouseDisplay();updateHistoryDisplay();updateAdminData();createCoinRain();updateLastResetDate();updateGlobalHistoryDisplay();populateUserWarehouseSelect();updateCoinOperationsDisplay();populateCoinOperationsFilterUser();setTimeout(()=>{const message=document.querySelector('.message');if(message)message.remove();},1000);bindModeButtons();currentPage='home';console.log('游戏初始化完成')}
...
document.getElementById('user-stats-section').style.display='block';userBalance.textContent=gameData.currentUser.balance;userBoxesOpened.textContent=gameData.currentUser.boxesOpened||0;const myHistory=(gameData.userHistory||[]).filter(r=>r.user_id===gameData.currentUser.id);const totalItemsFromHistory=myHistory.length;userWarehouse.textContent=totalItemsFromHistory;userLevel.textContent=gameData.currentUser.level;const warehouseNavLink=document.querySelector('.nav-link[data-page="warehouse"]');const warehouseMenuItem=document.querySelector('.sidebar-menu-item[data-page="warehouse"]');if(warehouseNavLink)warehouseNavLink.style.display='block';if(warehouseMenuItem)warehouseMenuItem.style.display='block'}
...
break;case 'gift':if(!amount||amount<=0){showMessage('请输入大于0的金币数量','error');return}
newBalance+=amount;operationText=`赠送 ${amount} 金币`;break;default:return}
try{await supabase.from('users').update({balance:newBalance,last_login:new Date().toLocaleString()}).eq('id',userId);setTimeout(()=>{const coinOperationsSection=document.getElementById('adminCoinOperationsSection');if(coinOperationsSection)coinOperationsSection.scrollIntoView({behavior:'smooth'});coinOperationsCurrentPage=1;updateCoinOperationsDisplay()},500);const operationRecord={admin_id:gameData.currentUser.id,admin_username:gameData.currentUser.username,user_id:userId,username:user.username,operation_type:operation,amount:amount,old_balance:user.balance,new_balance:newBalance,remark:remark||`管理员 ${gameData.currentUser.username} ${operationText}`};const{error:recordError}=await supabase.from('coin_operations').insert([operationRecord]);if(recordError)console.error('记录金币操作失败:',recordError);user.balance=newBalance;if(gameData.currentUser&&gameData.currentUser.id===userId){gameData.currentUser.balance=newBalance;updateUserUI()}
