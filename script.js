...
redeemAllBtn?.addEventListener('click',()=>{redeemType='all';showRedeemConfirmModal('all')});const searchInput=document.getElementById('searchUsernameInput');if(searchInput){searchInput.addEventListener('input',(e)=>{const keyword=e.target.value;populateCoinsUserSelect(keyword)})}
...
const coinsOperationSelect=document.getElementById('coinsOperation');if(coinsOperationSelect){coinsOperationSelect.addEventListener('change',function(){const operation=this.value;const coinsAmountInput=document.getElementById('coinsAmount');if(operation==='add'){coinsAmountInput.value=20000}else if(operation==='set'){coinsAmountInput.value=0}else{coinsAmountInput.value=20000}})}
redeemSelectedBtn?.addEventListener('click',()=>{const selectedItems=getSelectedWarehouseItems();if(selectedItems.length===0){showMessage('请先选择要兑换的物品','warning');return}
redeemType='selected';selectedItemsForRedeem=selectedItems;showRedeemConfirmModal('selected')});confirmRedeemBtn?.addEventListener('click',()=>{if(redeemType==='all'){redeemAllItems()}else if(redeemType==='selected'){redeemSelectedItems()}
...
function updateWarehouseDisplay(){if(!gameData.currentUser||gameData.currentUser.role==='admin'||gameData.currentUser.role==='subadmin')return;const warehouse=gameData.currentUser.warehouse||{};const itemsList=Object.keys(warehouse);const totalItems=itemsList.reduce((sum,itemId)=>{const count=warehouse[itemId];return sum+(typeof count==='number'?count:0)},0);warehouseCount.textContent=totalItems;let totalValue=0;itemsList.forEach(itemId=>{const item=gameData.items.find(i=>i.id===parseInt(itemId));if(item){totalValue+=item.value*warehouse[itemId]}});warehouseTotalValue.textContent=totalValue;warehouseTotalGold.textContent=totalValue;if(totalItems>0){redeemAllBtn.disabled=!1;redeemSelectedBtn.disabled=!1}else{redeemAllBtn.disabled=!0;redeemSelectedBtn.disabled=!0}
...
if(operation==='add'||operation==='subtract'){if(!amount||amount<=0){showMessage('请输入大于0的金币数量','error');return}}
...
newBalance-=amount;operationText=`减少 ${amount} 金币`;break;case 'set':newBalance=0;const actualSubtractAmount=user.balance;if(actualSubtractAmount>0){operationText=`清空金币（原余额 ${user.balance} 金币）`;amount=actualSubtractAmount;operation='subtract'}else{showMessage(`用户 ${user.username} 金币已经是 0，无需清空`,'info');modifyCoinsModal.classList.remove('active');return}
break}
