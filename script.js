let coinOperationsCurrentPage=1;let coinOperationsPageSize=20;let coinOperationsFilteredData=[];let isBoxOpening=!1;let isSavingItem=!1;let bgMusic=null;let isMusicPlaying=!1;let chatMessages=[];let onlineUsers=[];let heartbeatInterval=null;let chatSubscription=null;let onlineUsersSubscription=null;let unreadChatCount=0;function updateChatUnreadBadge(){const badge=document.getElementById('chatUnreadBadge');if(badge){badge.textContent=unreadChatCount;badge.style.display=unreadChatCount>0?'inline-block':'none'}}
...
showMessage(data.message,'success')}catch(error){console.error('清空系统数据异常:',error);showMessage('清空失败: '+error.message,'error')}}
...
if(gameData.currentUser&&(gameData.currentUser.role==='admin'||gameData.currentUser.role==='subadmin')){updateCoinOperationsDisplay()}}catch(error){console.error('处理金币操作记录更新时出错:',error)}}
...
updateRoundStats();updateAdminHomeStats();if(!isBoxOpening){enableAllBoxes()}}else if(eventType==='INSERT'){showMessage('恭喜所有用户完成了上一轮所有宝藏盒子！系统已自动重置，开始新的一轮！','success');setTimeout(async()=>{try{const roundResponse=await supabase.from('global_rounds').select('*').order('round_id',{ascending:!1}).limit(1).single();if(!roundResponse.error&&roundResponse.data){const roundData=roundResponse.data;gameData.globalRound={id:roundData.id,round_id:roundData.round_id,boxes:roundData.boxes||[],started_at:roundData.started_at,completed_at:roundData.completed_at,preset_generated:roundData.preset_generated||!1};if(gameData.currentUser){const homeSection=document.getElementById('home-section');const isHomePage=homeSection&&homeSection.classList.contains('active');const userRole=gameData.currentUser.role;if(isHomePage){if(userRole==='admin'){renderTreasureBoxesForAdmin();updateAdminData()}else if(userRole==='subadmin'){renderTreasureBoxesForSubadmin();updateAdminData()}else{renderTreasureBoxes()}
updateRoundStats()}
if(userRole==='admin'||userRole==='subadmin'){updateAdminHomeStats()}
updateGlobalHistoryDisplay();showMessage('新轮次数据加载完成！','success')}}}catch(error){console.error('加载新轮次数据失败:',error)}},1500)}}catch(error){console.error('处理轮次更新时出错:',error)}}
...
console.log('初始化游戏...');showMessage('正在加载游戏数据...','info');await loadGameData();if(!gameData.currentUser){userLoggedOut.style.display='block';userLoggedIn.style.display='none';document.getElementById('sidebar').style.display='none';showPage('home')}
bindEvents();updateWarehouseDisplay();updateHistoryDisplay();updateAdminData();createCoinRain();updateLastResetDate();updateGlobalHistoryDisplay();populateUserWarehouseSelect();updateCoinOperationsDisplay();populateCoinOperationsFilterUser();setTimeout(()=>{const message=document.querySelector('.message');if(message)message.remove();},1000);currentPage='home';console.log('游戏初始化完成')}
...
redeemConfirmModal.classList.remove('active')});refreshGlobalHistoryBtn?.addEventListener('click',()=>{updateGlobalHistoryDisplay();showMessage('全局开盒记录已刷新','success')});clearGlobalHistoryBtn?.addEventListener('click',async()=>{if(confirm('确定要清空所有开盒记录吗？此操作不可撤销！')){try{const{error}=await supabase.from('user_history').delete().neq('id',0);if(error)throw error;gameData.userHistory=[];updateGlobalHistoryDisplay();updateHistoryDisplay();showMessage('所有开盒记录已清空','success')}catch(error){console.error('Error clearing history:',error);showMessage('清空记录失败','error')}}});userWarehouseSelect?.addEventListener('change',function(){const userId=parseInt(this.value);if(userId){showUserWarehouse(userId)}else{selectedUserWarehouse.style.display='none'}});if(adminPanelToggle){adminPanelToggle.addEventListener('click',toggleAdminPanel)}
if(panelModifyCoinsBtn){panelModifyCoinsBtn.addEventListener('click',()=>{populateCoinsUserSelect();modifyCoinsModal.classList.add('active');closeAdminPanel()})}
...
closeAdminPanel()})}
if(panelResetUnopenedBtn){panelResetUnopenedBtn.addEventListener('click',()=>{if(confirm('确定、。')){resetUnopenedBoxes();showMessage('已重置','success')}
closeAdminPanel()})}
...
if(panelClearHistoryBtn){panelClearHistoryBtn.addEventListener('click',async()=>{if(confirm('确定要清空所有开盒记录吗？此操作不可撤销！')){try{const{error}=await supabase.from('user_history').delete().neq('id',0);if(error)throw error;gameData.userHistory=[];updateGlobalHistoryDisplay();updateHistoryDisplay();showMessage('所有开盒记录已清空','success')}catch(error){console.error('Error clearing history:',error);showMessage('清空记录失败','error')}}
closeAdminPanel()})}
...
box.presetItemId=selectedItemId;await saveRoundToSupabase(gameData.globalRound);renderTreasureBoxesForAdmin();let message='';if(selectedItemId===0){message=`宝盒 #${box.id} 已设置为空盒`;if(oldItem){message+=`，物品 ${oldItem.name} 已放回物品池`}}else{const item=gameData.items.find(i=>i.id===selectedItemId);if(item){message=`宝盒 #${box.id} 的奖品已更换为 ${item.name} (${item.value}金币)`;if(oldItem){message+=`，原物品 ${oldItem.name} 已放回物品池`}}}
showMessage(message,'success');editBoxItemModal.classList.remove('active');currentEditingBoxId=null;updateAdminHomeStats();if(document.getElementById('admin-section').classList.contains('active')){updateAdminData()}}catch(error){console.error('保存宝盒奖品失败:',error);showMessage('保存失败，请稍后重试','error')}}
...
function showUserWarehouse(userId){const user=gameData.users.find(u=>u.id===userId);if(!user)return;selectedUserName.textContent=user.username;const totalItems=Object.values(user.warehouse||{}).reduce((sum,count)=>sum+count,0);selectedUserItemCount.textContent=`${totalItems}个`;selectedUserWarehouseItems.innerHTML='';if(totalItems===0){selectedUserWarehouseItems.innerHTML=`
            <div style="grid-column: 1 / -1; text-align: center; padding: 30px; color: rgba(255, 215, 0, 0.7);">
                ...
                <h4 style="font-size: 1.2rem; margin-bottom: 10px;">仓库空空如也</h4>
                <p>该用户还没有收集到任何物品。</p>
            </div>
        `}else{Object.keys(user.warehouse||{}).forEach(itemId=>{const item=gameData.items.find(i=>i.id===parseInt(itemId));if(item){const quantity=user.warehouse[itemId];const totalValue=item.value*quantity;const rarityColor=getRarityColor(item.rarity);const itemElement=document.createElement('div');itemElement.className='user-warehouse-item';itemElement.innerHTML=`
                    ...
                    <div class="user-item-info">
                        ...
                        <div class="user-item-details">
                            ...
                            <div>总价值: <strong>${totalValue} 金币</strong></div>
                            <div>稀有度: ${getRarityText(item.rarity)}</div>
                        </div>
                    </div>
                    <div class="user-item-count">×${quantity}</div>
...
try{const result=await callRPC('register_user',{p_username:username,p_email:email,p_password:password});if(!result.success){showMessage(result.message,'error');return}
...
boxElement.dataset.boxId=box.id;let boxLabel='';let boxSubtitle='';if(isOpenedByCurrentUser){boxLabel='✓';boxSubtitle='已开启'}else if(isOpenedByOthers){boxLabel='🔒';boxSubtitle='已被开启'}else{boxLabel='';boxSubtitle='点击开启'}
boxElement.innerHTML=`
            <div class="box-glow"></div>
            <div class="box-decoration">
                <div class="box-corner corner-tl"></div>
...
if(!boxElement)return;isBoxOpening=!0;const allBoxes=document.querySelectorAll('.treasure-box');allBoxes.forEach(box=>{if(!box.classList.contains('opened')){box.style.cursor='not-allowed';box.style.opacity='0.6'}});try{const{data:result,error}=await supabase.rpc('open_box_function',{p_user_id:gameData.currentUser.id,p_box_id:boxId});if(error){console.error('开盒函数调用失败:',error);showMessage('开盒失败，请稍后重试','error');isBoxOpening=!1;enableAllBoxes();return}
...
function showResultModal(item,boxId){const rarityText=getRarityText(item.rarity);const rarityColor=getRarityColor(item.rarity);let itemContent='';if(item.cover_url){itemContent=`
...
showMessage(`已重置，开始第${newRoundId}轮！`,'success')}catch(error){console.error('Error resetting global round:',error);showMessage('重置失败，请稍后重试','error')}}
...
if(!warehouseItemsList)return;warehouseItemsList.innerHTML='';if(itemsList.length===0){warehouseItemsList.innerHTML=`
            <div style="text-align: center; padding: 50px; color: rgba(255, 215, 0, 0.7);">
                ...
                <h3 style="font-size: 1.5rem; margin-bottom: 10px;">仓库空空如也</h3>
                <p>快去开启宝藏盒子收集物品吧！</p>
            </div>
        `;return}
...
const itemElement=document.createElement('div');itemElement.className='result-item';itemElement.style.position='relative';itemElement.innerHTML=`
                ...
                ${iconHTML}
                <div class="item-info">
                    <div class="item-name">${item.name} <span class="warehouse-item-count">×${quantity}</span></div>
                    <div class="item-value">单件价值 ${item.value} 金币 | 总价值 <strong>${totalValue} 金币</strong></div>
                    <div class="item-rarity rarity-${item.rarity}">${getRarityText(item.rarity)} 物品</div>
                    <p style="margin-top: 10px; color: rgba(255, 215, 0, 0.9);">${item.description}</p>
                    <div style="margin-top: 15px;">
                        <button class="redeem-btn" data-item-id="${item.id}">兑换 ${item.value} 金币</button>
                        ${quantity > 1 ? `<button class="redeem-btn" data-item-id="${item.id}" data-quantity="all" style="background: rgba(46, 204, 113, 0.3);">兑换全部(${totalValue}金币)</button>` : ''}
                    </div>
                </div>
...
const warehouse=gameData.currentUser.warehouse||{};let totalGold=0;const redeemedItems=[];Object.keys(warehouse).forEach(itemId=>{const item=gameData.items.find(i=>i.id===parseInt(itemId));if(item){const quantity=warehouse[itemId];const itemTotalGold=item.value*quantity;totalGold+=itemTotalGold;redeemedItems.push({id:item.id,name:item.name,value:item.value,quantity:quantity,total:itemTotalGold})}});try{const{error:userError}=await supabase.from('users').update({balance:gameData.currentUser.balance+totalGold,warehouse:{},last_login:new Date().toLocaleString()}).eq('id',gameData.currentUser.id);if(userError)throw userError;gameData.currentUser.balance+=totalGold;gameData.currentUser.warehouse={};updateUserUI();updateWarehouseDisplay();showMessage(`成功兑换全部物品，获得 ${totalGold} 金币！`,'success')}catch(error){console.error('Error redeeming all items:',error);showMessage('兑换失败，请稍后重试','error')}}
...
function updateHistoryDisplay(){if(!historyTableBody)return;historyTableBody.innerHTML='';const userHistory=gameData.userHistory||[];const allHistory=[...userHistory].sort((a,b)=>new Date(b.obtained_at)-new Date(a.obtained_at)).slice(0,50);if(allHistory.length===0){historyTableBody.innerHTML=`
            <tr>
                <td colspan="5" style="text-align: center; color: rgba(255, 215, 0, 0.9);">暂无开盒记录</td>
            </tr>
        `;return}
...
if(adminUsersTable){adminUsersTable.innerHTML='';gameData.users.forEach(user=>{const row=document.createElement('tr');row.innerHTML=`
...
async function resetUnopenedBoxes(){try{showMessage('正在...','info');const{data:result,error}=await supabase.rpc('reset_unopened_boxes_by_admin');if(error){console.error('重置失败:',error);showMessage('重置失败：'+error.message,'error');return}
if(result&&!result.success){showMessage(result.message||'重置失败','error');return}
const{data:updatedRound,error:updateError}=await supabase.from('global_rounds').select('*').eq('id',gameData.globalRound.id).single();if(updateError){console.error('重新加载轮次数据失败:',updateError);showMessage('重置成功但更新显示失败','warning');return}
...
async function resetUnopenedBoxesByValue(targetValue){try{showMessage(`正在按 ${targetValue} ...`,'info');const{data,error}=await supabase.rpc('reset_unopened_boxes_by_value',{target_value:targetValue,tolerance_percent:10,enable_rollback:!0});if(error){throw error}
...
if(!confirm(`确定要拒绝用户 ${user.username} 的注册申请吗？此操作不可撤销。`))return;try{await supabase.from('users').delete().eq('id',userId);gameData.users=gameData.users.filter(u=>u.id!==userId);updateAdminData();showMessage(`用户 ${user.username} 的注册申请已拒绝`,'success')}catch(error){console.error('Error rejecting user:',error);showMessage('拒绝失败，请稍后重试','error')}}
...
function populateCoinsUserSelect(){if(!coinsUserSelect)return;coinsUserSelect.innerHTML='<option value="">请选择用户</option>';const allowedUsers=gameData.users.filter(user=>user.role==='user'&&user.approved);allowedUsers.forEach(user=>{const option=document.createElement('option');option.value=user.id;option.textContent=`${user.username} (当前金币 ${user.balance})`;coinsUserSelect.appendChild(option)})}
async function modifyCoins(){const userId=parseInt(coinsUserSelect.value);const operation=coinsOperation.value;const amount=parseInt(document.getElementById('coinsAmount').value);const remark=document.getElementById('coinsRemark').value;if(!userId||!amount||amount<=0){showMessage('请选择用户并输入有效的金币数量','error');return}
...
if(user.role==='admin'||user.role==='subadmin'){showMessage('不能修改管理员的金币','error');return}
let newBalance=user.balance;let operationText='';switch(operation){case 'add':newBalance+=amount;operationText=`增加 ${amount} 金币`;break;case 'subtract':if(user.balance<amount){showMessage('用户金币不足，无法减少','error');return}
newBalance-=amount;operationText=`减少 ${amount} 金币`;break;case 'set':newBalance=amount;operationText=`设置为 ${amount} 金币`;break}
...
modifyCoinsModal.classList.remove('active');showMessage(`成功${operationText}，用户 ${user.username} 当前金币为 ${newBalance}${remark ? `(备注:${remark})` : ''}`,'success');coinsUserSelect.value='';coinsOperation.value='add';document.getElementById('coinsAmount').value='500';document.getElementById('coinsRemark').value='';updateCoinOperationsDisplay()}catch(error){console.error('Error modifying coins:',error);showMessage('修改金币失败，请稍后重试','error')}}
...
records.forEach(record=>{const operationTypeText={'add':'增加','subtract':'减少','set':'设置'}[record.operation_type]||record.operation_type;const operationTypeClass={'add':'operation-add','subtract':'operation-subtract','set':'operation-set'}[record.operation_type]||'';let timeDisplay='';if(record.created_at){const recordDate=new Date(record.created_at);const today=new Date();if(recordDate.toDateString()===today.toDateString()){timeDisplay=recordDate.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}else{timeDisplay=recordDate.toLocaleString()}}
const row=document.createElement('tr');row.innerHTML=`
...
async function deleteCoinOperation(operationId){if(!confirm('确定要删除这条金币操作记录吗？此操作不可撤销！'))return;try{const{error}=await supabase.from('coin_operations').delete().eq('id',operationId);if(error)throw error;gameData.coinOperations=gameData.coinOperations.filter(record=>record.id!==operationId);updateCoinOperationsDisplay();showMessage('金币操作记录已删除','success')}catch(error){console.error('删除金币操作记录失败:',error);showMessage('删除失败，请稍后重试','error')}}
async function clearAllCoinOperations(){if(!confirm('确定要清空所有金币操作记录吗？此操作不可撤销！'))return;try{const{error}=await supabase.from('coin_operations').delete().neq('id',0);if(error)throw error;gameData.coinOperations=[];updateCoinOperationsDisplay();showMessage('所有金币操作记录已清空','success')}catch(error){console.error('清空金币操作记录失败:',error);showMessage('清空失败，请稍后重试','error')}}
